...
async def test_manager_can_configure_the_cold_drink_surcharge(
    client: AsyncClient, login: Callable[..., Awaitable[dict[str, Any]]]
) -> None:
    await login(role_name="MANAGER")
    response = await client.put(
        "/api/v1/settings/pos/cold-drink-surcharge", json={"amount": "0.35"}
    )
    assert response.status_code == 200
    assert response.json()["value"] == "0.35"


async def test_cashier_cannot_read_or_change_the_cold_drink_surcharge(
    client: AsyncClient, login: Callable[..., Awaitable[dict[str, Any]]]
) -> None:
    ...
    assert (
        await client.put("/api/v1/settings/pos/cold-drink-surcharge", json={"amount": "0.15"})
    ).status_code == 403
